...
export default function Canvas({ roomId }: { roomId: string }) {
  ...
  // Update both Liveblocks storage and room metadata
  const onTitleChange = async (newTitle: string) => {
    // Update Liveblocks storage (handled by BloqContainer's mutation)
    // Update room metadata for document list
  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 space-y-6">
      <div className="flex gap-4 justify-center">
        <FloatingToolbar disabled={!isReady} />
      </div>
        ...
        <div className="relative z-10 p-8 flex flex-col min-h-[650px]">
          {!isReady ? (
            ...
            <BloqContainer
              ...
              onUpdate={() => {
                // Updates are handled by BloqContainer internally
              }}
              onRemove={() => {
                // Removal is handled by BloqContainer internally
              }}
            />
          )}
        </div>
